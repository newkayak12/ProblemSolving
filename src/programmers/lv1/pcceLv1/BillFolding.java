...
public class BillFolding {
    ...
    static Stream<Arguments> exampleSource() {
        return Stream.of(
            Arguments.of(new Example(new int[]{30, 15}, new int[]{26, 17}, 1)),
            Arguments.of(new Example(new int[]{50, 50}, new int[]{100, 241}, 4))
        );
    }
    ...
    void test(Example example) {
        int[] wallet = example.wallet();
        int[] bill = example.bill();

        assertEquals(solution(wallet, bill), example.result());
    }
