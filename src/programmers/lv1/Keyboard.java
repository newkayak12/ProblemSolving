...
public class Keyboard {
     ...
     *
     *  1번 키부터 차례대로 할당된 문자들이 순서대로 담긴 문자열 배열
     *  keymap과 입력하려는 문자열들이 담긴 문자열 배열 targets가 주어질 때,
    ...
    class TestCases {
        ...
        public void case1() {
            ...
            int[] result = {9,4};
            Assertions.assertArrayEquals(result, solution(keymap, targets));
    ...
    public int[] solution( String[] keymap, String[] targets ) {
        ...
        List<List<String>> keyMaps = Arrays.stream(keymap)
                                            .map(elem -> Arrays.stream(elem.split(""))
                                                               .collect(Collectors.toList()))
                                            .collect(Collectors.toList());

        for ( int i = 0; i < targets.length; i ++  ) {
            int result = findKeyIndex(targets[i], keyMaps);
            answer[i] = result;
        }
    ...
    private int findKeyIndex ( String input, List<List<String>>  keyMaps) {
       int count = 0;

        loop : for ( String key : input.split("") ) {
            int result = keyMaps.stream()
                    .filter(inner -> inner.contains(key))
                    .mapToInt(inner -> (inner.indexOf(key) + 1))
                    .sorted()
