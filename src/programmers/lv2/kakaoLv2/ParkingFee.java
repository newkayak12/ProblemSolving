...
public class ParkingFee {
    ...
    public int[] solution(int[] fees, String[] records) {
        ...
        for ( String record : records ) {
            ...
            timeMap.putIfAbsent(number, new LinkedList<>());
            long timestamp = 0;

            if( "IN".equalsIgnoreCase(type) ) timestamp = -1 * time.getLong(ChronoField.MINUTE_OF_DAY);
            if( "OUT".equalsIgnoreCase(type)) timestamp = time.getLong(ChronoField.MINUTE_OF_DAY);


            List<Long> timeSet = timeMap.get(number);
            timeSet.add(timestamp);
            timeMap.put(number, timeSet);
        }


        for ( String number : timeMap.keySet() ) {
            if( timeMap.get(number).size() % 2 != 0 ) {
                List<Long> timeSet = timeMap.get(number);
                timeSet.add(LocalTime.parse("23:59").getLong(ChronoField.MINUTE_OF_DAY));
                timeMap.put(number, timeSet);
            }
        }
        ...
        int[] answer = timeMap.keySet().stream()
            ...
            int fee = baseFee;
            totalMin -= baseTime;
            if(totalMin > 0)    fee +=  (int)Math.ceil(totalMin / (perMinutes*1.0) ) *  perFee;
            return fee;
        })
        .mapToInt(i -> i)
        .toArray();
